...
std::vector<std::pair<std::pair<int, int>, std::string>> Car::vehiclesLocationArray;

Car::Car(Building& inBuilding, std::pair<int, int> location,  std::vector<std::pair<std::pair<int, int>, std::pair<std::string, std::string>>> intersectionsList, int speed = 17)
    : inBuilding_(inBuilding), location_(location), intersectionsList_(intersectionsList), currentSpeed_(speed) {
        maxCapacity_ = 4;
        idxVehicleArray_ = vehiclesLocationArray.size();
        vehiclesLocationArray.push_back(std::make_pair(location_,getDirection()));
...
const std::vector<std::pair<int, int>> Car::determineRoute_(Building *origin, Building *arrive) {
...
const std::vector<std::pair<int,int>>& Car::getRoute() const {return route_;}

const std::string Car::getDirection() const {
...
void Car::update_determine_route(std::vector<std::pair<int,int>> original_route)
{
...
void Car::drive(Building* origin, Building* destination, std::vector<TrafficLight*> trafficLight_list){
...
bool Car::isIntersection_(std::pair<int, int> crds) {
...
bool Car::isOccupied_(std::pair<int, int> crds) {
    for (auto v : vehiclesLocationArray) {
        if( (v.first.first == crds.first) && (v.first.second == crds.second)){
            if (getDirection() == v.second) {return true;}}
    }
    return false;
}

const std::pair<int, int> & Car::getLocation() const{
    return location_;
}
const int& Car::getCurrentSpeed() const{
    return currentSpeed_;
}

const int& Car::getIndexVehicleArray() const{
    return idxVehicleArray_;
}
const int& Car::getMaxCapacity() const{
    return maxCapacity_;
}

const std::vector<Person*>& Car::getPassengers() const{
    return passengers_;
}
Building* const Car::getDestination() const{
    return destination_;
}
const std::vector<std::pair<int, int> > Car::getRoute(){
    return route_;
}
const std::vector<std::pair<std::pair<int, int>, std::pair<std::string, std::string>>> Car::getIntersectionsList(){
    return intersectionsList_;
}

Building& Car::getBuilding(){
    return inBuilding_;
} 


const std::vector<std::pair<int,int>> Car::get_updated_determine_route() const
{
    return updated_determine_route_;
}

// Setters:
void Car::setDestination(Building* des){
    destination_ = des;
}
void Car::updateLocation(std::pair<int, int> newLocation) {
    location_ = newLocation;
    vehiclesLocationArray[idxVehicleArray_] = std::make_pair(newLocation, getDirection());
}
...
//memeber fucntions:
void Car::getInto(Building& the_garage_of) {
    inBuilding_ = the_garage_of;
    updateLocation (inBuilding_.getCoordinateArray().at(1));
}

void Car::accommodate(Person* p) {
    passengers_.push_back(p);
}

void Car::evict(const std::string passengerName) {
...
void Car::evictAll() { passengers_.clear(); }


bool Car::nodeInList(std::pair<int, int> node, std::vector<std::pair<int, int>> list){
    int count = 0;
...
TrafficLight* Car::findTrafficLight (std::vector<TrafficLight*> trafficLight_list, std::pair<int, int> coordinates)
{
